...
public class NetworkController {
    ...
    public void callApiFile(Context activity, String url, Map<String, String> mapParams, Map<String, String> mapFiles, Map<String, List<String>> mapFilesList, final String tag, final Bundle objectExtras, final APICallbacks apiCallbacks) {
        ...
        /*if (objectMultiFiles.length() > 0) {
                JSONArray arrayParams = objectMultiFiles.names();
                Log.i("ParamKeys", objectMultiFiles.toString());
                Log.i("ParamKeys", arrayParams.toString());
                for (int i = 0; i < arrayParams.length(); i++) {
                    String key = arrayParams.getString(i);
                    JSONArray arrayFiles = objectParams.getJSONArray(key);

                    for (int j = 0; j < arrayFiles.length(); j++) {
                        Log.i("ParamKeys" + j, arrayFiles.getString(i));
                        multiPartBuilder.addMultipartFile(key, new File(arrayFiles.getString(i)));
                    }
                }
            }*/


        //multiPartBuilder.addHeaders("Authorization", "Bearer " + activity.getSharedPreferences(SHARED_PREF, 0).getString("Authorization", ""));
      /*  if (activity != null)
